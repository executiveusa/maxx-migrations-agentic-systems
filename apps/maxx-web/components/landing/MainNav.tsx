...
export function MainNav() {
  ...
  return (
    <header className="sticky top-0 z-40 border-b border-border/80 bg-bg/90 backdrop-blur-xl supports-[backdrop-filter]:bg-bg/80">
      <nav aria-label="Primary" className="relative mx-auto flex max-w-7xl items-center justify-between px-5 py-3 md:px-8">
        <Link href="/" aria-label="MACS Digital Media home" className="inline-flex items-center gap-3 active:scale-[0.99]">
        ...
        <div className="hidden items-center gap-5 lg:flex">
          <ul className="flex items-center gap-5 xl:gap-7">
            {links.map((link) => (
              <li key={link.href}>
                <Link href={link.href} className="text-sm text-muted transition-colors duration-150 hover:text-text active:text-accent">
                  {link.label}
                </Link>
              </li>
            ))}
        ...
        <button
          ...
          className="min-h-11 rounded-full border border-border px-4 py-2 text-sm transition-transform duration-150 active:scale-[0.98] lg:hidden"
        >
        ...
        {mobileOpen && (
          <div id="mobile-nav-menu" className="absolute left-0 right-0 top-full border-b border-border bg-bg px-5 py-5 shadow-lg lg:hidden">
            <ul className="space-y-1">
              {links.map((link) => (
                <li key={link.href}>
                  <Link href={link.href} onClick={() => setMobileOpen(false)} className="block min-h-12 border-b border-border py-4 text-base">
                    {link.label}
                  </Link>
                </li>
              ))}
            </ul>
            <Link href="/audit" onClick={() => setMobileOpen(false)} className="mt-5 block min-h-12 rounded-full bg-accent px-5 py-3 text-center text-sm font-bold text-white">
              Diagnose it · $497
            </Link>
          </div>
        )}
