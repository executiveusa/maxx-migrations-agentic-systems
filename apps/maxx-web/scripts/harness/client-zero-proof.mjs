...
const payload = {
  ...
  answers: {
    ...
    outcome_90: { value: "Prove durable intake and approval-gated exactly-once execution." },
  },
  icm: { identity: [{ id: "company_name", value: "MACS Client Zero Harness" }] },
  ontology: { entities: ["Company"], relationships: [], states: [], events: ["IntakeSubmitted"] },
  open_questions: [],
...
if (!submit.response.ok) {
  console.error("Client Zero submit failed:", submit.response.status, submit.data);
  process.exit(1);
}
...
if (!receipt.response.ok) {
  console.error("Receipt lookup failed:", receipt.response.status, receipt.data);
  process.exit(1);
}

if (receipt.data?.proof?.sideEffects !== 0 || !receipt.data?.proof?.pendingApproval) {
  console.error("FAIL: proposal is not cleanly pending approval.", receipt.data);
  process.exit(1);
}
